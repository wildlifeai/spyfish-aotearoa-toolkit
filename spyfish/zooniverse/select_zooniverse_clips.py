...
def process_zooniverse_clips(maxn_csv_path, output_selections_path, drop_id, config):
    ...
    if not dep_row or dep_row['sampling_start'] is None or dep_row['sampling_end'] is None:
        raise ValueError(f"Missing mandatory sampling metadata (SamplingStart/End) for {drop_id}.")
    ...
    if df.empty:
        ...
        selector = ClipSelector(drop_id, sampling_start, config.zooniverse_clip_length)
        duration = sampling_end - sampling_start
        if duration > 0:
            interval_step = duration / config.zooniverse_health_check_count
            for i in range(1, config.zooniverse_health_check_count):
        ...
        df[config.csv_time_seconds_column] = df[config.csv_maxn_time_column].apply(time_to_seconds)

        unique_species = df[config.csv_scientific_name_column].unique()
        is_binary = len(unique_species) <= 1 or config.zooniverse_force_binary_strategy

        if is_binary:
            strategy_params = config.zooniverse_binary_strategy
        else:
            strategy_params = config.zooniverse_multiclass_strategy

        selections_df = select_clips_with_strategy(
            df=df,
            drop_id=drop_id,
            sampling_start=sampling_start,
            clip_length=config.zooniverse_clip_length,
            strategy_params=strategy_params,
            is_multiclass=not is_binary,
            video_start_threshold=config.zooniverse_video_start_threshold,
            clip_cap=config.zooniverse_clip_cap
        )
    ...
    logging.info(f"Zooniverse Extractions computed: {len(selections_df)} unique clips selected.")
