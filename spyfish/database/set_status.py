...
def main():
    ...
    valid_statuses = [attr for attr in dir(PipelineStatus) if attr.isupper() and not attr.startswith("_") and isinstance(getattr(PipelineStatus, attr), str)]

    parser = argparse.ArgumentParser(description="Manually update a deployment status in the Spyfish pipeline database.")
    parser.add_argument("drop_id", help="The DropID to update (e.g. KSF_20240124_BUV_KSF_085_01)")
    parser.add_argument("status", help=f"The new status. Valid options: {valid_statuses}")
    ...
    if args.status not in valid_statuses:
        logging.error(f"Invalid status '{args.status}'. Must be one of: {valid_statuses}")
        sys.exit(1)
