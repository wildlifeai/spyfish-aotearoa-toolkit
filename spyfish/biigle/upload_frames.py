...
def upload_coco_annotations_to_biigle(
    volume_id: int,
    coco_json_path: str,
    label_id: int = config.biigle_default_fish_label_id,
) -> dict:
    """
    ...
    All bboxes use `label_id` (defaults to `config.biigle_default_fish_label_id`).
    ...
    if not Path(coco_json_path).exists():
        logging.warning(f"COCO JSON not found at {coco_json_path}. Skipping annotation upload.")
        return {}
    ...
    coco_img_id_to_filename = {img["id"]: img["file_name"] for img in coco_data.get("images", [])}
    ...
    label_mapping = config.biigle_label_mapping or {}
...
def upload_frames_to_biigle(
    ...
    if volume_id:
        logging.info(f"✅ Biigle Volume Created! Link: https://biigle.de/projects/{project_id or config.biigle_project_id}/volumes/{volume_id}")
