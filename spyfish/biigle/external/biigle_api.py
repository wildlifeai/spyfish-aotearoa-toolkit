...
class Api(object):
    def __init__(
        ...
        """
        email = email or os.environ.get("BIIGLE_API_EMAIL", "")
        token = token or os.environ.get("BIIGLE_API_TOKEN", "")
        if not email or not token:
            raise ValueError(
                "BIIGLE API credentials are required. Provide email and token "
                "or set BIIGLE_API_EMAIL / BIIGLE_API_TOKEN environment variables."
            )
