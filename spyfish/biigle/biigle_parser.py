...
class BiigleParser:
    ...
    def _export_report_with_cache(
        ...
        if use_cache and cache_path.exists():
            ...
            report_id = self.biigle_handler.create_report(resource, resource_id, type_id)
        ...
        dfs = self.biigle_handler.read_csvs_from_zip_bytes(zip_bytes, allow_nested=allow_nested)
        if not dfs:
            return pd.DataFrame()

        result_df = pd.concat(dfs.values(), ignore_index=True)
        logging.info(f"Loaded {len(dfs)} CSV(s), {len(result_df)} total rows from {resource} {resource_id}")
        return result_df
    ...
    def process_video_annotations(
        ...
        if resource == "local":
            ...
            annotations_df = self._export_report_with_cache(
                resource=resource, resource_id=volume_id, type_id=type_id, use_cache=use_cache
            )
    ...
    def process_30s_count(self, annotations_df: pd.DataFrame) -> pd.DataFrame:
        ...
        return grouped.sort_values(["start_seconds", "frame_seconds"]).reset_index(drop=True)
    ...
    def process_sizes(self, annotations_df: pd.DataFrame) -> pd.DataFrame:
        ...
        scale_size = sizes_df[sizes_df["label_name"] == SCALE_BAR_LABEL_NAME]["size_px"].mean()
    ...
    def extract_time_values(self, annotations_df: pd.DataFrame) -> pd.DataFrame:
        ...
        total_seconds = annotations_df["start_seconds"] + annotations_df["frame_seconds"]
        annotations_df["time_of_max"] = (
            pd.to_datetime(total_seconds, unit="s").dt.strftime("%H:%M:%S")
        )
