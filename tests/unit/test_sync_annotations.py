...
Covers the pure data-transformation layer — no Biigle API calls.
  ...
  - _aggregate_annotations: MaxN counting, multiple species, sorting
"""
...
def test_map_strips_common_name_prefix():
    ...
    key, mapped = _map_biigle_to_spyfish_schema(
        row, "label_name", "KSF_20240124_BUV_KSF_085_01", "00:01:00.000"
    )
...
def test_map_plain_species_name():
    ...
    _, mapped = _map_biigle_to_spyfish_schema(
        row, "label_name", "DROP_01", "00:00:05.000"
    )
...
def test_map_annotation_id_fallback():
    ...
    _, mapped = _map_biigle_to_spyfish_schema(row, "label_name", "DROP_01", None)
    assert mapped["external_id"] == "77"


def test_map_null_timestamp():
    _, mapped = _map_biigle_to_spyfish_schema(
    ...
    assert mapped["time_of_max"] is None
...
def _make_biigle_df(rows: list[dict]) -> pd.DataFrame:
    """Build a DataFrame matching the shape _aggregate_annotations expects."""
...
def test_aggregate_counts_per_timestamp_species():
    ...
    result = _aggregate_annotations(df, "KSF_20240124_BUV_KSF_085_01")
    assert len(result) == 1
...
def test_aggregate_multiple_species():
    ...
    result = _aggregate_annotations(df, "DROP_01")
    assert len(result) == 2
